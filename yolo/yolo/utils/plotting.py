...
from yolo.utils import LOGGER, TryExcept, ops, plt_settings, threaded
...
@plt_settings()
def plot_results_with_extra_loss(file='path/to/results.csv', dir='', segment=False, pose=False, classify=False, on_plot=None):
    """
    Plot training results from a results CSV file. The function supports various types of data including segmentation,
    pose estimation, and classification. Plots are saved as 'results.png' in the directory where the CSV is located.
    ...
    Example:
        ```python
        from dayolo.utils.plotting import plot_results
    ...
    if classify:
        ...
        index = [1, 2, 3, 4, 5, 6, 9, 10, 11, 12, 7, 8]
    ax = ax.ravel()
    files = list(save_dir.glob('results*.csv'))
    assert len(files), f'No results.csv files found in {save_dir.resolve()}, nothing to plot.'
    for f in files:
...
@plt_settings()
def plot_results_with_extra_losses(file='path/to/results.csv', dir='', segment=False, pose=False, classify=False, on_plot=None):
    """
    Plot training results from a results CSV file. The function supports various types of data including segmentation,
    pose estimation, and classification. Plots are saved as 'results.png' in the directory where the CSV is located.
    ...
    if classify:
        ...
        fig, ax = plt.subplots(2, 6, figsize=(12, 6), tight_layout=True)
        index = [1, 2, 3, 4, 7, 8, 11, 12, 13, 14, 9, 10]
    ax = ax.ravel()
    files = list(save_dir.glob('results*.csv'))
    assert len(files), f'No results.csv files found in {save_dir.resolve()}, nothing to plot.'
    for f in files:
