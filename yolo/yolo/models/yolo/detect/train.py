...
from yolo.nn.tasks import DetectionModel, DomainClassifier, MultiDomainClassifier, FeaturesDistance, MultiFeaturesSingleDomainClassifier, UnsupervisedDomainClassifier, UnsupervisedMultiDomainClassifier, InstanceDomainClassifier
...
from yolo.nn.modules import (GradReversal, Conv_, AdaptiveAvgPooling, DomainClassifierNetwork, DomainClassifierNetwork_m, DomainClassifierNetwork_s, multiFeatDomainClassifierNetwork, FeaturesUnspacifier, FeaturesDomainClassifierNetwork)
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            #for i, batch in pbar:
                ...
                with torch.cuda.amp.autocast(self.amp):
                    ...
                    # Compute DA losses
                    dc_loss_s = self.model.args.dc[0] * dc_loss_criterion(domains_pred_s, domain_target).unsqueeze(0)
                    dc_loss_m = self.model.args.dc[0] * dc_loss_criterion(domains_pred_m, domain_target).unsqueeze(0) 
                    dc_loss_l = self.model.args.dc[1] * dc_loss_criterion(domains_pred_l, domain_target).unsqueeze(0)
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, batch in pbar:
                ...
                with torch.cuda.amp.autocast(self.amp):
                    ...
                    # Run DA
                    domains_pred_s = DomainClassifierNetwork_s(features[0])
                    domains_pred_m = DomainClassifierNetwork_m(features[1])
                    domains_pred_l = DomainClassifierNetwork(features[2])
                    ...
                    # Compute DA losses
                    dc_loss_s = self.model.args.dc[0] * dc_loss_criterion(domains_pred_s, domain_target).unsqueeze(0)
                    dc_loss_m = self.model.args.dc[1] * dc_loss_criterion(domains_pred_m, domain_target).unsqueeze(0)
                    dc_loss_l = self.model.args.dc[2] * dc_loss_criterion(domains_pred_l, domain_target).unsqueeze(0)

                    # Combien all losses
                    self.loss = tmp_loss + (dc_loss_s + dc_loss_m + dc_loss_l) * len(source_batch['im_file'])
                    self.loss_items = torch.cat((tmp_loss_items, dc_loss_s, dc_loss_m, dc_loss_l), dim=0)
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, batch in pbar:
                ...
                with torch.cuda.amp.autocast(self.amp):
                    ...
                    # Run DA subnetwork
                    features_s, features_m, features_l = features[0], features[1], features[2]
                    domains_pred = multiFeatDomainClassifierNetwork(features_s, features_m, features_l) 
...
class UnsupervisedMultiFeaturesSingleDomainClassifierTrainer(BaseTrainer):
    """
    A class extending the BaseTrainer class for training based on a detection model.
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            #for i, batch in pbar:
                ...
                with torch.cuda.amp.autocast(self.amp):
                    ...
                    # Save DA accuracy
                    domain_preds_label = domains_pred.max(1).indices.cpu()
                    acc_ = sklearn.metrics.accuracy_score(domain_target.cpu(), domain_preds_label, normalize=True)
                    acc_list = [epoch, i, acc_]
                    with open(os.path.join(self.model.args.save_dir,'dc_accuracy.csv'), mode='a+') as dc_acc:
                        writer_obj = writer(dc_acc)
                        writer_obj.writerow(acc_list)
                        dc_acc.close()
                    ...
                    # Compute DA loss
                    dc_loss_ = self.model.args.dc * dc_loss_criterion(domains_pred, domain_target).unsqueeze(0)
                    
                    # Combine all losses
                    self.loss = tmp_loss + (dc_loss_)*len(target_batch['im_file'])
                    self.loss_items = torch.cat((tmp_loss_items, dc_loss_), dim=0)
...
class FeaturesDistanceTrainer(BaseTrainer):
    ...
    def separate_source_target_features(self, img_file_names, features):
        ...
        for i, img in enumerate(img_file_names):
            if os.path.basename(img).startswith(self.model.args.source_name):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, batch in pbar:
                ...
                with torch.cuda.amp.autocast(self.amp):
                    ...
                    # Retireve non spatial features
                    features = FeaturesUnspacifier(features)
                    # separate source & target features
                    source_features, target_features = self.separate_source_target_features(source_batch['im_file'], features)

                    # Compute L2-norm
                    features_dist = self.model.args.dc * torch.cdist(source_features, target_features, p=2).mean().unsqueeze(0)

                    # Combine all losses
                    self.loss = tmp_loss + features_dist*len(batch['im_file'])
                    self.loss_items = torch.cat((tmp_loss_items, features_dist), dim=0)
...
class UnsupervisedFeaturesDistanceTrainer(BaseTrainer):
    """
    A class extending the BaseTrainer class for training based on a detection model.
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            #for i, batch in pbar:
                ...
                with torch.cuda.amp.autocast(self.amp):
                    ...
                    # Retireve non spatial features
                    source_features = FeaturesUnspacifier(source_features)
                    ...
                    # Combine all losses
                    self.loss = tmp_loss + features_dist*len(target_batch['im_file'])
                    self.loss_items = torch.cat((tmp_loss_items, features_dist), dim=0)
...
class InstanceDomainClassifierTrainer(BaseTrainer):
    """
    A class extending the BaseTrainer class for training based on a detection model.
    ...
    def get_model(self, cfg=None, weights=None, verbose=True):
        """Return a YOLO detection model."""
        model = InstanceDomainClassifier(cfg, nc=self.data['nc'], verbose=verbose and RANK == -1)
        if weights:
            model.load(weights)
        return model
    ...
    def _do_train(self, world_size=1):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, batch in pbar:
                ...
                with torch.cuda.amp.autocast(self.amp):
                    source_batch = self.preprocess_batch(batch)
                    features, bboxes, tmp_loss, tmp_loss_items = self.model(source_batch)
                    dc_loss_criterion = nn.CrossEntropyLoss(reduction='mean')
                    ...
                    bboxes = torch.unbind(bboxes, dim=0)
                    selected_features = torchvision.ops.roi_pool(input=features[0:4], boxes=bboxes[0:4], output_size=10)
                    del features
                    torch.cuda.empty_cache()
                    print("selected_features", selected_features.size())
                    instances_domainpreds = FeaturesDomainClassifierNetwork(selected_features)
                    print("instances_domainpreds", instances_domainpreds.size())
                    instances_domain_target = []
                    for i in range(4): # range(len(bboxes)):
                        for j in range(bboxes[i].size()[0]):
                            instances_domain_target.append(domain_target[i])
                    instances_domain_target = torch.stack(instances_domain_target).type(torch.LongTensor).to('cuda')
                    fdc_loss = self.model.args.dc * dc_loss_criterion(instances_domainpreds, instances_domain_target).unsqueeze(0)


                    self.loss = tmp_loss + dc_loss*len(source_batch['im_file']) + fdc_loss*len(source_batch['im_file'])
                    self.loss_items = torch.cat((tmp_loss_items, dc_loss, fdc_loss), dim=0)
