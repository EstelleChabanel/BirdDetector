...
parser.add_argument('--dataset-list', type=list_of_strings, required=False)
...
if args.dataset_list:
    SUBDATASETS = [x for x in EVAL_DATASETS_MAPPING[DATASET_NAME]['source'] if x in args.dataset_list]
else:
    SUBDATASETS = EVAL_DATASETS_MAPPING[DATASET_NAME]['source']

print(SUBDATASETS, type(SUBDATASETS), type(SUBDATASETS[0]))
eps = 1e-8
...
model = YOLO(MODEL_PATH) #.load(MODEL_PATH)
